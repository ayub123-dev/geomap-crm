...
class CustomerExistingService
{
    ...
    public function update($id, array $payload, $userId = null)
    {
        ...
        // cek duplicate kode_existing untuk data lain selain data yang sedang diupdate
        if (
    $this->repository->existsByKodeExistingExceptId(
        $payload['kode_existing'],
        $id
    )
) {
    throw new InvalidArgumentException(
        'Kode existing sudah digunakan oleh data lain.'
    );
}
       // fungsi $data berfungsi untuk menyiapkan data yang akan disimpan ke database dengan memanggil metode preparePayload() yang akan memproses payload input dan menghasilkan array data yang siap disimpan.
       // Kemudian, data tersebut akan diteruskan ke metode update() dari repository untuk melakukan
    ...
    // jika mode commit, maka data akan disimpan ke database. jika mode preview, 
    // maka data hanya akan diproses tanpa disimpan
    if ($execute) {
// cek duplicate kode_existing
    if ($this->repository->existsByKodeExisting($payload['kode_existing'])) {
        ...
        $result[] = array(
            'row' => $index + 1,
            'status' => 'duplicate',
            'message' => 'Kode existing sudah ada, data di-skip.',
            'data' => $payload,
        );
    ...
    // catat log audit untuk setiap baris yang berhasil diimpor
    $this->audit->log(
        $userId,
        'customer_existing',
        'import_create',
        ...
        json_encode($payload)
                ...
                // berfungsi untuk menghitung jumlah baris yang berhasil diimpor dan yang gagal, 
                //serta menyimpan hasilnya dalam array $result untuk setiap baris yang diproses. 
                //Jika proses impor dijalankan (execute = true), maka data akan disimpan ke database dan log audit akan dicatat untuk setiap baris yang berhasil diimpor. Jika terjadi duplikasi kode_existing, 
                //maka baris tersebut akan di-skip dan dihitung sebagai kegagalan dengan status 'duplicate'.
                $success++;
                $result[] = array(
                    'row' => $index + 1,
                    'status' => 'ok',
