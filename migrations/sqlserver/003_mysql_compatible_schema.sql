...
-- SQL Server schema yang kompatibel dengan struktur MySQL modul utama
-- dan tetap idempotent agar aman dijalankan berulang.
...
BEGIN
    CREATE TABLE dbo.cities (
        ...
        name NVARCHAR(150) NOT NULL,
        city_type NVARCHAR(50) NOT NULL CONSTRAINT df_cities_city_type DEFAULT 'kota' CONSTRAINT ck_cities_city_type CHECK (city_type IN ('kabupaten', 'kota')),
        geojson NVARCHAR(MAX) NULL,
...
BEGIN
    CREATE TABLE dbo.role_user (
        ...
        created_at DATETIME2 NOT NULL CONSTRAINT df_role_user_created_at DEFAULT GETDATE(),
...
BEGIN
    CREATE TABLE dbo.permission_role (
        ...
        role_id INT NOT NULL,
        created_at DATETIME2 NOT NULL CONSTRAINT df_permission_role_created_at DEFAULT GETDATE(),
...
BEGIN
    CREATE TABLE dbo.salesman_wilayah (
        ...
        village_id INT NULL,
        coverage_type NVARCHAR(50) NOT NULL CONSTRAINT df_salesman_wilayah_coverage_type DEFAULT 'primary' CONSTRAINT ck_salesman_wilayah_coverage_type CHECK (coverage_type IN ('primary', 'secondary', 'temporary')),
...
BEGIN
    CREATE TABLE dbo.customer_poi_nearby (
        ...
        poi_type NVARCHAR(50) NOT NULL CONSTRAINT df_customer_poi_type DEFAULT 'lainnya' CONSTRAINT ck_customer_poi_type CHECK (poi_type IN ('kompetitor', 'fasilitas_umum', 'transportasi', 'perumahan', 'lainnya')),
...
BEGIN
    CREATE TABLE dbo.kunjungan (
        ...
        jenis_kunjungan NVARCHAR(50) NOT NULL CONSTRAINT df_kunjungan_jenis DEFAULT 'followup' CONSTRAINT ck_kunjungan_jenis CHECK (jenis_kunjungan IN ('prospek', 'followup', 'penagihan', 'complaint', 'retensi', 'lainnya')),
        status NVARCHAR(50) NOT NULL CONSTRAINT df_kunjungan_status DEFAULT 'planned' CONSTRAINT ck_kunjungan_status CHECK (status IN ('planned', 'checked_in', 'completed', 'cancelled')),
...
BEGIN
    CREATE TABLE dbo.prospek_history (
        ...
        customer_inti_id INT NOT NULL,
        salesman_id INT NULL,
        stage_from NVARCHAR(50) NOT NULL CONSTRAINT df_prospek_history_stage_from DEFAULT 'lead' CONSTRAINT ck_prospek_history_stage_from CHECK (stage_from IN ('lead', 'qualified', 'proposal', 'negotiation', 'won', 'lost')),
        stage_to NVARCHAR(50) NOT NULL CONSTRAINT df_prospek_history_stage_to DEFAULT 'qualified' CONSTRAINT ck_prospek_history_stage_to CHECK (stage_to IN ('lead', 'qualified', 'proposal', 'negotiation', 'won', 'lost')),
...
BEGIN
    CREATE TABLE dbo.target_realisasi (
        ...
        tahun INT NOT NULL,
        bulan INT NOT NULL,
        target_kunjungan INT NOT NULL CONSTRAINT df_target_realisasi_target DEFAULT 0,
        realisasi_kunjungan INT NOT NULL CONSTRAINT df_target_realisasi_realisasi DEFAULT 0,
        created_at DATETIME2 NOT NULL CONSTRAINT df_target_realisasi_created_at DEFAULT GETDATE(),
        updated_at DATETIME2 NOT NULL CONSTRAINT df_target_realisasi_updated_at DEFAULT GETDATE(),
        CONSTRAINT uq_target_realisasi_salesman_periode UNIQUE (salesman_id, tahun, bulan),
        CONSTRAINT fk_target_realisasi_salesman FOREIGN KEY (salesman_id) REFERENCES dbo.salesman(id) ON DELETE CASCADE
    );
END;
GO
...
BEGIN
    CREATE TABLE dbo.notifikasi (
        ...
        channel NVARCHAR(50) NOT NULL CONSTRAINT df_notifikasi_channel DEFAULT 'in-app' CONSTRAINT ck_notifikasi_channel CHECK (channel IN ('in-app', 'whatsapp')),
        status NVARCHAR(50) NOT NULL CONSTRAINT df_notifikasi_status DEFAULT 'draft' CONSTRAINT ck_notifikasi_status CHECK (status IN ('draft', 'queued', 'sent', 'failed')),
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_permissions_module' AND object_id = OBJECT_ID('dbo.permissions'))
