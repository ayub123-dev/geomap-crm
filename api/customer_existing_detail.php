...
try {
    if ($method === 'GET') {
        Rbac::authorize('customer_existing.view');
    ...
    if ($method === 'PUT') {
        Rbac::authorize('customer_existing.update');
        $payload = Request::data();
        $result = $service->update($id, $payload, $userId);
        if (!$result) {
            Response::json(array('success' => false, 'message' => 'Data tidak ditemukan.'), 404);
        }
        Response::json(array('success' => true, 'message' => 'Customer existing diperbarui.', 'data' => $result));
    }

    if ($method === 'DELETE') {
        Rbac::authorize('customer_existing.delete');
        $deleted = $service->delete($id, $userId);
        Response::json(array('success' => $deleted, 'message' => $deleted ? 'Data dihapus.' : 'Gagal menghapus.'));
    }
