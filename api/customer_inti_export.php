...
try {
    ...
    if ($format === 'pdf') {
        if (class_exists('\\Dompdf\\Dompdf')) {
            $html = '<h2>GeoMap CRM - Export Customer Inti</h2><table border="1" cellspacing="0" cellpadding="4"><tr>';
            ...
            header('Content-Disposition: attachment; filename="customer_inti_' . date('Ymd_His') . '.pdf"');
            echo $dompdf->output();
            exit;
        }
    ...
    if (class_exists('\\PhpOffice\\PhpSpreadsheet\\Spreadsheet')) {
        ...
        header('Content-Disposition: attachment; filename="customer_inti_' . date('Ymd_His') . '.xlsx"');
        $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
        $writer->save('php://output');
        exit;
    }

    ImportExport::downloadCsv('customer_inti_' . date('Ymd_His') . '.csv', $headers, $rows);
